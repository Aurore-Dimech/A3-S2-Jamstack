...
const HeroCard = ({hero}) => {
    console.log(hero)
    return (
        <Link href={`/heroes/${hero.uid}`} className="w-full flex justify-center border border-white rounded-xl overflow-hidden gap-3">
            <img src={`http://localhost:1337${hero.image.url}`} alt="" className="rounded-xl max-h-52 lg:max-h-64"/>
